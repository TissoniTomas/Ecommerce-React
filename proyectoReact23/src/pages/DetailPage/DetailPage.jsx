...
const DetailPage = () => {
  ...
  return (
    <main className={`h-[1400px] ${mode === "light"? "bg-white" : "bg-gray-900"}`}>
      <div className="flex flex-col items-center text-center relative ">
        <img className="w-72" src={image} alt={title} />
        <div className="flex flex-col h-screen items-center m-10">
          <h1 className={`font-Montserrat text-3xl hover:bg-black hover:text-white mt-20 lg:text-5xl ${mode === "light" ? "text-gray-900" : "text-white"}`}>
            {title}
          </h1>

          <span className="my-10 text-3xl font-Montserrat text-sky-500 lg:text-5xl lg:my-14">{price}</span>
          <p className={`my-20 font-Inter text-lg mx-6 lg:text-2xl lg:mx-20 ${mode=== "light" ? "text-gray-900" : "text-gray-400"}`}>{description}</p>
          <form action="">

          <div className="flex justify-evenly w-screen lg:w-[60rem]">
            <label className={`font-Montserrat mx-6 lg:text-2xl ${mode=== "light" ? "text-gray-900" : "text-gray-400"}`} htmlFor="quantity">
              Select Quantity
            </label>
            <input
              className={`w-20 font-Montserrat lg:text-2xl focus:border-4 focus:rounded-lg focus:animate-fade-up focus:border-sky-500 focus:outline-none ${mode === "light"? "bg-white text-gray-900" : "bg-gray-900 text-gray-400"}`}
              type="number"
              name="stock"
              id="stock"
              value={counterStock}
            ...
            <svg
              ...
              strokeWidth={1.5}
              stroke="currentColor"
              className={`w-6 h-6 lg:h-10 lg:w-10 cursor-pointer ${mode=== "light" ? "text-gray-900" : "text-gray-400"}`}
          ...
          <div className="flex justify-around items-center w-screen my-20 lg:w-[60rem]">
            ...
            <label className={`font-Montserrat mx-6 lg:text-2xl ${mode=== "light" ? "text-gray-900" : "text-gray-400"}`} htmlFor="size">
              Select Size
            </label>
              <select
                className={`w-20 font-Montserrat lg:text-2xl focus:border-4 focus:rounded-lg focus:animate-fade-up focus:border-sky-500 focus:outline-none ${mode === "light"? "bg-white text-gray-900" : "bg-gray-900 text-gray-400"}`}
